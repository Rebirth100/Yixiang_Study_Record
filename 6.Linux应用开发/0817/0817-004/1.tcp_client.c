...
//  ./client  192.168.x.x  9000
int main(int argc,char **argv)
{
    char peerAdddress[20]="127.0.0.1";
    char peerPort[8]="9000";
    if(argc==3)
    {
        strcpy(peerAdddress,argv[1]);
        strcpy(peerPort,argv[2]);
    }
    ...
    fprintf(stderr, "socket成功  fd=%d 连接服务器%s:%s\n", socketFd,peerAdddress,peerPort);
    struct sockaddr_in serverAddress;
    serverAddress.sin_family = AF_INET;
    serverAddress.sin_port = htons(atoi(peerPort));
    serverAddress.sin_addr.s_addr = inet_addr(peerAdddress);
