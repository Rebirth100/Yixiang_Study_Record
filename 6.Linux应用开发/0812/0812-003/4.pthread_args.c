...
int main()
{
    srand(time(0));
    pthread_t ptid;
    int status = rand() % 50 + 100;
    pthread_create(&ptid, NULL, routine, (void *)&status);
    ...
    if (pthread_join(ptid, (void **)&end) == 0)
    {
        ...
        if(*end==status)
        {
            fprintf(stderr,"子线程业务执行完毕，无错误 *end=%d\n",*end);
        }
        else
        {
            fprintf(stderr,"子线程业务执行有错误 *end=%d\n",*end);
        }
    }
    else
    {
